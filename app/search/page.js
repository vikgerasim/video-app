  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        // Fetch items from each category type
        await Promise.all(
  ...
  useEffect(() => {
    const fetchVideos = async () => {
      ...
      try {
        ...
        // If we have a search query, handle it differently
        if (searchQuery?.trim()) {
          // Convert search query to lowercase
          const searchLower = searchQuery.toLowerCase();
          
          // First, get all videos (with any active filters)
          ...
          const videoData = snapshot.docs
            ...
            .filter(video => {
              ...
              const makeMatch = video.carMake?.toLowerCase().includes(searchLower);
              const modelMatch = video.carModel?.toLowerCase().includes(searchLower);
              const categoryMatch = video.systemCategory?.toLowerCase().includes(searchLower);
              const workTypeMatch = video.workType?.toLowerCase().includes(searchLower);
              
              return titleMatch || makeMatch || modelMatch || categoryMatch || workTypeMatch;
            })
            .sort((a, b) => b.createdAt - a.createdAt);

          setVideos(videoData);
        } else {
          // If no search query, just get all videos with filters and sort by date
          const q = query(videosRef, ...constraints, orderBy('createdAt', 'desc'));
          const snapshot = await getDocs(q);
      ...
      {Object.values(filters).some(f => f) && (
        <div className="flex flex-wrap gap-2 mb-8">
          {Object.entries(filters).map(([key, value]) => {
            ...
            const category = categories[key === 'systemCategory' ? 'systemCategories' : 
                                     key === 'workType' ? 'workTypes' : 
                                     key === 'carMake' ? 'carMakes' : null];
